...
I had to switch to using mediapipe 0.10.30 instead of 0.10.13. The older version required me to force install certain versions of numpy and torch, which inside Colab would have required the TA to restart the session after updated packages were downloaded. To avoid this, I needed to pick a pose processor that was compatible with the module versions that come stock in Colab.
...
* first run after migrating versions (on local GPU): 81.61% accuracy on class balance set (not bad, not great), , likely chalked up to "lite" pose estimator
* first migrated run in colab
    - videos take 11ish seconds to estimate poses on 
    - 84.80% final accuracy 