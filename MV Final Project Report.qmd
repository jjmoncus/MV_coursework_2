...
* Trying to train directly on raw videos produced poor results.
...
* If you can justifiably change your regression problem into a classification problem, then the learning task likely becomes much easier.
...
To get a sense of our training data, I watched most of our training videos and inspected some metadata:
  ...
  - Videos are shot from all angles (from the side, from the front, from above), and sometimes with camera movement. Any feature learning we do will have to take into account how the camera affects the subject's position in plane-space.

  - Since pushups are relatively smooth movements, we may be able to stride over frames (i.e. for each vide,  only look at every 5th or 10th frame) during both training and inference and still see the subject's movement. This way, we could save a lot of memory overhead.
...
In the end, I defaulted to pose estimation, as a way of working with significantly smaller data that was still informative to our task. But first, below is a record of what I tried:
...
I conducted training with the following fixed parameters:
...
Below is the summary of the first model, which I then modified a number of ways in different combinations:
...
* Validation accuracy is capped at 60% because the mean 
* These 10 epochs took 10 minutes to run, and the model was consuming about 6.54GB VRAM on my Macbook's MPS the whole time, i.e. painful.
...
After several failed attempts, I thought a small ResNet with skip connections might do solve the vanishing gradients problem. The skip connections make the loss landscape smoother, which makes calculating gradients easier.

So I replaced the spatial feature stage with a pre-built ResNet18, beginning with all weights frozen to avoid the computational overhead of computing so many more gradients.
...
I tried a series of modificaitons to see if this ResNet backbone could successfully tackle the problem:
...
**Result**: 
 ...
 I refroze all ResNet-backbone weights going forward.
...
Now that the time cost of training was so low, I could play with several tweaks:
  ...
  - Adding small FC layer before temporal convolutions
    ...
    - Adding an FC layer with the same in and out channels made the model worse. 
      ...
      - **Takeaway:** Layers with the same input and output channels can be dangerous. The model is liable to learn non-useful representations, like rotation, translation, or simply noise.
...
## 2.2.1) Small Modificaitons:

- **3D**: exported pose esimations using 3-D coordinates instead of 2-D (still provided by mediapipe). The 3-D locations are in theory more robust to different camera angles.
- **Smoothing** Up to this point, the majority of mistakes tended to be overcounts, i.e the model seeing push-ups where there are none. This is likely due to the model getting confused by small movements. We can smooth the joint coordinates using SavGol smoothing with a moderate window_size (I chose 7) and moderate polynomial degree (I chose 2) to even out small, unhelpful movements. However, too large a smoothing window, and you risk washing out movements you actually want, whereas too large a polynomial degree, and you risk retaining jitters that aren't meaningful.
...
The final training dataset consisted of 658 videos, with the following class balance:
 ...
 66, 116,  68,  42,  72,  72,  44,  42,  68,  68]])
...
This architecture acheives 94% accuracy on the original training videos, as well as 92% accuracy on our class-balanced set.
...
The confusion matrix (on the class-balanced training set) shows this model still struggles to distinguish between 9 and 10 pushups.
