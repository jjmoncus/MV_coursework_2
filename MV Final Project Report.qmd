...
* Always, always, **always** check that module versions are the same across training/inference environments.

*Note*: Unless otherwise noted, all published numbers refer to training/inference runs on my local machine. See final section for comments when trying to run in Colab.
...
# Final Update: Discrepancy Between Colab and Local MPS

Upon trying to run my notebooks to Colab, I was getting inexplicably terrible errors. Despite working well on my local machine, the model would seemingly only predict values of 1, 2, and 3 in the inference notebook, resulting in 3-5% final accuracy.

After some review, I discovered the discrepancy was due to different versions of modules downloaded in each environment.
...
For safety's sake, I decided to load my environment with all of Colab's defaults for the following library versions:
...
The biggest hurdle was the `numpy` switch, since the updated version was no longer compatible with mediapipe 0.10.13 that I had been using for pose estimation. I had to switch to mediapipe 0.10.30, which required rewriting the functions that would load and estimate poses on videos. Continuing to use 0.10.13 would have required a force-install of a different numpy version in Colab, which would have then required the TA to restart the Colab session after updated packages were downloaded, which would broke the rules of the assignment.

Some obeservations about mediapipe 0.10.30:
...
* I chose "lite" instead of "heavy" model for speed - heavy took 15 seconds per video, lite takes 4-5 (both on local GPU).

**Results:**

* first run (on local GPU) after migrating module versions: 
    * 82% accuracy on class balance set
    * 90% accuracy on original videos. 
    * The decrease in accuracy come from using the "lite" pose estimator, instead of the "heavy" one.

* first run in colab (on CPU, since I ran out of T4 credits):
    * 84.80% final accuracy on class balanced set
    * 89.61% accuracy on original videos.

Below is the Summary screenshot from Colab.

![Summary of Inference, Final](plots/Summary_of_Inference_FINAL_FINAL.png){fig-align="center" width=80%}
